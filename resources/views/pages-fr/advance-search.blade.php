                ...
                <div class="row">
                    <div class="col-md-12">
                        <h2>{{ str_replace('-', ' ', request()->keyword) }}
                            {{ request()->country ? '/ ' . $countryNameFr : '' }}
                            {{ request()->city ? '/ ' . $cityNameFr : '' }}
                            {{ request()->topic ? '/ ' . $topicNameFr : '' }}
                            {{ request()->month ? '/ ' . $monthNameFr : '' }}</h2>
                    </div>
                </div>
            </div>
        </section>
                                ...
                                <div class="col-sm-12 topic-midule-grid">
                                    <div class="topic-date-cnfr">
                                        <div class="county-conference">
                                            <div class="col-md-12">
                                                <h1 style=" font-size: 18px;font-family:Gill Sans;">À venir
                                                    {{ request()->keyword }} en Europe {{ date('Y') }}</h1>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="eve-prom">
                                    ...
                                    <div class="p-4 mt-4" style="background-color: #f5f7fd: font-weight: 700">
                                        ...
                                        @else
                                            <h4 class="text-capitalize fs-3">Aucun résultat pour {{ request()->keyword }}
                                                {{ request()->country ? '/ ' . $countryNameFr : '' }}
                                                {{ request()->city ? '/ ' . $cityNameFr : '' }}
                                                {{ request()->topic ? '/ ' . $topicNameFr : '' }}
                                                {{ request()->month ? '/ ' . $monthNameFr : '' }}.
                                            </h4>
                                            <p>Vérifiez votre orthographe ou utilisez des termes plus généraux
                                            </p>
                                        @endif
                                    </div>
