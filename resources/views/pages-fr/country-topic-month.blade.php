...
@section('meta')
    <title>
        {{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['title']) }}
    </title>
    <meta name="keyword"
        content="{{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['metaKey']) }}" />
    <meta name="description"
        content="{{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['metaDes']) }}" />

    <meta property="og:title"
        content="{{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['title']) }}" />
    ...
    <div class="tm-main-content" id="top">
        <section class="global-page-header">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <h2>{{ $countryNameFr }} / {{ $topicNameFr }} / {{ $monthNameFr }}</h2>
                    </div>
                </div>
            </div>
        </section>
                                        ...
                                        <div class="county-conference">
                                            <div class="col-md-12">
                                                <h1 style=" font-size: 18px;font-family:Gill Sans;">
                                                    {{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['h1']) }}
                                                </h1>
                                                <p>{{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['contentOne']) }}
                                                </p>
                                                <p>{{ str_replace(['@country', '@topic', '@month'], [$countryNameFr, $topicNameFr, $monthNameFr], $content['contentTwo']) }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#load-more-btn').on('click', function(event) {
                ...
                fetchLoadMoreEvents(country, topic, month, page);
            });

            // load more button for mobile
            function fetchLoadMoreEvents(country, topic, month, page) {
                $.ajax({
                    url: "{{ route('country-topic-month-ajax') }}",
                    type: "GET",
                    data: {
                        country: country,
                        topic: topic,
